...
@Name=provider.init.create_table_provider
...
(
	ID 					INT GENERATED BY DEFAULT AS IDENTITY(START WITH 1) PRIMARY KEY,
	CUR_1				VARCHAR(3) NOT NULL,
	CUR_2				VARCHAR(3) NOT NULL,
	CONSTRAINT			CURRENCY_PAIR_UNIQUE UNIQUE(CUR_1, CUR_2)
)
...
#-- DUAL comme dans oracle, une table avec une seule colonne et un seul enregistrement
@Name=provider.init.create_table_dual
CREATE TABLE DUAL
(
	DUMMY				VARCHAR(1)
)

#-- Will throw an exception if the dual table has not been created, showing us that
#-- schema initialization went wrong
@Name=provider.init.check_existance
SELECT * FROM DUAL;
